defmodule Processes do
  ...
  koan "Use tail recursion to receive multiple messages" do
    ...
    send(pid, {self(), "o"})
    assert_receive "O", 100, "sorry this msg not found"

    send(pid, {self(), "hai"})
    assert_receive "HAI", 100, "sorry this msg not found"
  end
  ...
  koan "Processes can be used to hold state" do
    ...
    send(pid, {self(), :get})
    assert_receive "foo", 100, "error"

    send(pid, {self(), :set, "bar"})
    send(pid, {self(), :get})
    assert_receive "bar", 100, "error"
  end
  ...
  koan "Waiting for a message can get boring" do
    ...
    assert_receive {:waited_too_long, "I am impatient"}, 100, "error"
  end
